...
if( !function_exists( 'wcusage_enqueue_frontend_ajax' ) ) {
 function wcusage_enqueue_frontend_ajax() {
   ...
   $dashboard_page = $options['wcusage_dashboard_page'];
   ...
   if( function_exists( 'is_product' ) ) {
     if( ( !is_front_page() && !is_product() ) || $post_id == $dashboard_page ) {
       if( $post_id == $dashboard_page || $post_id == $mla_dashboard_page || is_account_page() || wcusage_page_contain_shortcode($post_id) ) {
         ...
         // wp_enqueue_script( 'jquery', WCUSAGE_UNIQUE_PLUGIN_URL .'js/jquery.js', array(), '0' );
         wp_enqueue_script( 'jquery', '', array(), '0' );
         wp_localize_script( 'jquery', "wcusageajax", $variables );

         // Remove javascript defer from pages that include main plugin shortcode.
...
if( !function_exists( 'wcusage_fix_defer_js' ) ) {
  function wcusage_fix_defer_js() {
    ...
    if ( is_plugin_active( 'wp-rocket/wp-rocket.php' ) || is_plugin_active( 'perfmatters/perfmatters.php' ) ) {
      $post_id = get_the_ID();
      $options = get_option( 'wcusage_options' );
      $dashboard_page = $options['wcusage_dashboard_page'];
      $mla_dashboard_page = wcusage_get_setting_value('wcusage_mla_dashboard_page', '');
...
if( !function_exists( 'wcusage_check_admin_access' ) ) {
  function wcusage_check_admin_access() {
  	...
  	if( current_user_can($wcusage_field_order_sort) || current_user_can('administrator') ) {
  		return true;
  	} else {
  		return false;
  	}
...
if( !function_exists( 'wcusage_check_lifetime_or_coupon' ) ) {
  function wcusage_check_lifetime_or_coupon($order_id, $coupon_code) {
  	$wcu_lifetime_referrer = get_post_meta( $order_id, 'lifetime_affiliate_coupon_referrer', true );
...
if( !function_exists( 'wcusage_convert_order_value_to_currency' ) ) {
  function wcusage_convert_order_value_to_currency($orderinfo, $the_value) {
    ...
    if($orderinfo) {
      ...
      $wcusage_currency_conversion = get_post_meta( $orderinfo->get_id(), 'wcusage_currency_conversion', true );
...
if( !function_exists( 'wcusage_ajax_error' ) ) {
  function wcusage_ajax_error() {
    ...
    $ajaxerrormessage = "<br/><span style='color: red; font-weight: bold;'>" . __( "ERROR: Failed to load ajax request. Session may have timed out. Refresh the page to try again.", "woo-coupon-usage" ) . "</span>";
    if(current_user_can( 'edit_posts' )) {
      $ajaxerrormessage .= "<br/>Admin: If this keeps happening, <a href='/wp-admin/admin.php?page=wcusage_settings&section=tab-help' target='_blank'><strong>click here</strong></a> for more information.";
    }
