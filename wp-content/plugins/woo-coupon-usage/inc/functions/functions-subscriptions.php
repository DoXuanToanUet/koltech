    ...
    if ( !function_exists( 'wcusage_new_renewal_order' ) ) {
        function wcusage_new_renewal_order( $order, $subscription )
        {
            ...
            if ( $parent_order_id ) {
                ...
                if ( $parentorder ) {
                    $recurringaffiliateparent = $parentorder->get_meta( 'lifetime_affiliate_coupon_referrer' );
                    if ( $recurringaffiliateparent ) {
                        update_post_meta( $order->ID, 'lifetime_affiliate_coupon_referrer', $recurringaffiliateparent );
                    }
                }
    ...
    function wcusage_get_sub_order_icon( $orderid )
    {
        ...
        if ( is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
            ...
            if ( wcusage_is_order_renewal( $orderid ) ) {
                ...
                $subicon = '<i class="fas fa-sync" title="' . __( "Subscription Renewal Order", "woo-coupon-usage" ) . $showsubid . '" style="font-size: 10px;"></i> ';
            }
            
            if ( wcusage_is_order_parent( $orderid ) ) {
                $subicon = '<i class="fas fa-retweet" title="' . __( "New Subscription (Parent Order)", "woo-coupon-usage" ) . '" style="font-size: 10px;"></i> ';
            }
        }
