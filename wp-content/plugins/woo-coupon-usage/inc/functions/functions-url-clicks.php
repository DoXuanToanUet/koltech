...
if ( !function_exists( 'wcusage_install_clicks_data' ) ) {
    ...
    {
        $coupon_id = sanitize_text_field( $coupon_id );
        $campaign = sanitize_text_field( $campaign );
        $page = sanitize_text_field( $page );
        $refpage = sanitize_text_field( $refpage );
        $converted = sanitize_text_field( $converted );
        ...
        $wpdb->insert( $table_name, array(
            ...
            'converted' => $converted,
            'ipaddress' => $ipaddress,
            'date'      => get_date_from_gmt( date( 'Y/m/d H:i:00' ) ),
        ) );
...
if ( !function_exists( 'wcusage_display_coupon_url_clicks' ) ) {
    ...
    )
    {
        ...
        global  $wpdb ;
        $table_name = $wpdb->prefix . 'wcusage_clicks';
        $result2 = $wpdb->get_results( "SELECT * FROM " . $table_name . " WHERE couponid = " . $postid . $campaignline . $convertedline . " ORDER BY id DESC LIMIT " . $wcusage_field_show_click_history_amount . " OFFSET " . $offset );
    ...
    <!-- Heading -->
		<p style='font-size: 22px; float: left; margin-top: 20px;' id='wcu-recent-clicks-section'><?php 
        echo  __( 'Recent Clicks', 'woo-coupon-usage' ) ;
        ?>:</p>
    ...
    <?php 
        ...
        if ( $wcusage_field_load_ajax && $show_converted ) {
            ?>
    <p style="float: right; font-size: 17px; margin-top: 25px;">
      <label for="wcu-checkbox-clicks-converted" style="vertical-align: baseline;">
        <input type="checkbox" id="wcu-checkbox-clicks-converted" name="wcu-checkbox-clicks-converted" value="1" style=""> <?php 
            echo  __( 'Converted Only', 'woo-coupon-usage' ) ;
            ?>
      </label>
    </p>
    <?php 
    ...
    <?php 
        ...
        if ( $totalresults > 0 ) {
            ...
            echo  "<td class='wcuTableHead'>" . __( 'ID', 'woo-coupon-usage' ) . "</td>" ;
            echo  "<td class='wcuTableHead'><i class='fas fa-cart-plus' title='" . __( 'Converted?', 'woo-coupon-usage' ) . "'></i></td>" ;
            echo  "<td class='wcuTableHead' style='max-width: 300px;'>" . __( 'Landing Page', 'woo-coupon-usage' ) . "</td>" ;
            echo  "<td class='wcuTableHead' style='max-width: 350px;'>" . __( 'Referring URL', 'woo-coupon-usage' ) . "</td>" ;
            echo  "<td class='wcuTableHead'>" . __( 'Date', 'woo-coupon-usage' ) . "</td>" ;
            echo  "</tr>" ;
            foreach ( $result2 as $result ) {
                echo  "<tr class='wcuTableRow'>" ;
                echo  "<td class='wcuTableCell' style='max-width: 100%;'>" . $result->id . "</td>" ;
                
                if ( $result->converted == 1 ) {
                    $convertedicon = '<i class="fas fa-check" style="color: green;" title="' . __( "Converted", "woo-coupon-usage" ) . '"></i>';
                } else {
                    $convertedicon = '<i class="fas fa-times" title="' . __( "Not Converted", "woo-coupon-usage" ) . '"></i>';
                }
                
                echo  "<td class='wcuTableCell' style='max-width: 100%;'>" . $convertedicon . "</td>" ;
                echo  "<td class='wcuTableCell wcuTableCell-ref-landing'><a href='" . get_permalink( $result->page ) . "'>" . get_the_title( $result->page ) . "</a></td>" ;
                ...
                echo  "<td class='wcuTableCell wcuTableCell-ref-website'>" . $referrerurl . "</td>" ;
                $thedatetime = strtotime( $result->date );
                echo  "<td class='wcuTableCell' style='max-width: 100%;'>" . date_i18n( "M jS, Y (g:ia)", $thedatetime ) . "</td>" ;
                echo  "</tr>" ;
            }
            echo  "</table>" ;
        } else {
            ...
            if ( $page == 0 ) {
                ?>
			  <p><?php 
                echo  __( 'There have been no clicks for this campaign yet.', 'woo-coupon-usage' ) ;
                ?></p>
        <script>
        ...
        <?php 
                ...
                ?>
        <p><?php 
                echo  __( 'No clicks available.', 'woo-coupon-usage' ) ;
                ?></p>
        <?php 
            }
